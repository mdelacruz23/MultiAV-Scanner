...
def main():
    ...
    parser.add_argument('-b', '--bucket', help='S3 Bucket', default='data-dz-logs')
    ...
    parser.add_argument('-k', '--key', help='S3 key to be searched for', default=None)
    ...
    try:
        ...
        for key in key_list:
            ...
            if s3_key is not None:
                try:
                    s3.Bucket(bucket).download_file(key, SEARCH_FILE_NAME)
                except botocore.exceptions.ClientError as e:
                    if e.response['Error']['Code'] == "404":
                        logger.info(f"object {key} does not exist.")
                    else:
                        logger.warning(f"Failure to download object {key}")
                        return 1
            with open(SEARCH_FILE_NAME, 'r') as fp:
                for l_no, line in enumerate(fp):
                    ...
                    if s3_key in line:
                        ...
                        size_bytes = line_tokens[15]
                        logger.info(f"\nSEARCHED FILES: {counter} \nDROP TIME: {drop_time} \nDROP BUCKET: {drop_bucket} \nSIZE: {size_bytes} B \nAWS ACCOUNT: {AWS_Account} \nAWS LOGIN: {AWS_Login} \nREMOTE IP: {IP_Address}")
                        return 0
                    else:
                        continue
    ...
    logger.info('Retrieved {} key_list'.format(counter))
