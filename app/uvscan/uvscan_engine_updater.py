...
LOG_NAME = 'uvscan_engine_updater'
global log_level
log_level = LOG_LEVEL_DEF


def main():
    parser = argparse.ArgumentParser(description="McAfee AntiVirus: Engine Updater ")
    ...
    if not bucket or not prefix:
        parser.error("engine bucket and prefix must be provided")
    ...
    target = ("upload to ", "download from ")[download]
    global log
    prefix = prefix + '/' + SCANNER_NAME
    log = get_logger(LOG_NAME, log_level, LOG_ENGINE_FILENAME)
    log.info(f'log_level: {log_level}')
    log.info(f"Checking for uvscan engine updates to {target} s3 bucket: {bucket} prefix: {prefix}")
    while True:
