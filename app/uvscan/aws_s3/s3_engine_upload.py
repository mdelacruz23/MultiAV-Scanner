import os
# from aws_s3.common_defs import MCAFEE_ENGINE_FILE_PREFIXES
# from aws_s3.common_defs import MCAFEE_ENGINE_FILE_SUFFIXES
...
def s3_engine_upload(s3_client, s3, bucket, prefix, local_path):
    ...
    for avfile in avfiles:
        ...
        if (local_file_sha256 != s3_file_sha256):
            log.info(f"file: {avfile} local sha256: {local_file_sha256} s3 sha256: {s3_file_sha256}")
            ...
            try:
                ...
                log.info(f"Upload {SCANNER_NAME} virus definitions file {avfile} \
                      to s3 bucket: {bucket} Key: {prefix} completed at {get_timestamp()}")
            except Exception as e:
                log.error(f"Upload scanner {SCANNER_NAME} file {avfile} to s3 bucket: {s3_object.bucket_name} Key: {s3_object.key}  FAILED due to: {e}")

        else:
            log.info(f"Not uploading {avfile} because sha256 on remote matches local.")
