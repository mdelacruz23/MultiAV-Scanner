...
from aws_s3.common_defs import UVSCAN_TMP_DIR
from aws_s3.common_defs import UVSCAN_INSTALL_DIR
from aws_s3.common_defs import UVSCAN_DEFS_FILE_PREFIXES
from aws_s3.common_defs import UVSCAN_DEFS_FILE_SUFFIXES
...
def uvscan_defs_updater(bucket, prefix, download, uri, accesskey, secretkey):
    if uri:
    ...
    if download:
        ...
        if new_version:
            ...
            nDatabases = os.listdir(UVSCAN_TMP_DIR)
            for database in nDatabases:
                shutil.move(os.path.join(UVSCAN_TMP_DIR, database),
                            os.path.join(UVSCAN_INSTALL_DIR, database))
    else:
        log.info(f"Uploading virus definitions. Bucket: {bucket} Prefix: {prefix}")
        new_version = s3_upload(s3_client, s3, bucket, prefix, UVSCAN_TMP_DIR)
    return new_version


def download_defs(s3_client, s3, bucket, prefix):

    log.info("starting update uvscan virus definitions from s3")
    # check which definitions were updated
    to_download = s3_download(
        s3_client, bucket, prefix, UVSCAN_INSTALL_DIR,
        UVSCAN_TMP_DIR, UVSCAN_DEFS_FILE_PREFIXES, UVSCAN_DEFS_FILE_SUFFIXES
    )
