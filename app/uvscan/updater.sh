...
printf "$(date) -  $prgname: Performing an update ($current_version -> $major_ver.$minor_ver)\n"
...
[ -n "$new_version" ] \
|| exit_error "unable to get newwly installed DAT version."
new_major=`echo "$new_version" | cut -d. -f-1`
new_minor=`echo "$new_version" | cut -d. -f2-`

if [ "$new_major" = "$major_ver" -a "$new_minor" = "$minor_ver" ]
then printf "$(date) - $prgname: DAT update succeeded $current_version -> $new_version\n"
else exit_error "DAT update failed - installed version different than expected\n"
fi
else
	printf "$(date) - $prgname: DAT already up to date ($current_version)\n"
fi
