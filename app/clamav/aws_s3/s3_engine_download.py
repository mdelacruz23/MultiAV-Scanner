# -*- coding: utf-8 -*-
# A set of functions to download AV engine from S3
...
def s3_engine_download(s3_client, bucket, prefix, engine_path, tmp_engine_path, engine_prefix_set, engine_suffix_set):

    Path(tmp_engine_path).mkdir(parents=True, exist_ok=True)
    to_download = {}
    local_file_sha256 = 0
    engine_keys = []
    for key in get_s3_keys(s3_client, bucket, prefix, engine_suffix_set):
        log.info(f"Adding file for download {key}")
        engine_keys.append(key)
    log.info(f"Scanner {SCANNER_NAME} engine key set: {engine_keys}")
    for key in engine_keys:
        filename = os.path.basename(key)
        local_file_path = os.path.join(engine_path, filename)
        local_tmp_path = os.path.join(tmp_engine_path, filename)
        ...
        if os.path.exists(local_file_path):
            ...
            if local_file_sha256 == s3_sha256:
                log.info(f"Scanner {SCANNER_NAME} Not downloading {key} because local sha256 matches s3.")
                continue
        if s3_sha256:
...
def get_s3_keys(s3_client, bucket, prefix, suffix_set):
    ...
    while True:
        ...
        if 'Contents' in resp:
            ...
            log.error(f"Scanner {SCANNER_NAME} No key found in bucket : {bucket} prefix: {prefix} suffix_set: {suffix_set}")
