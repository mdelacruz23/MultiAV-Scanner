...
from aws_s3.clamav_defs_updater import clamav_defs_updater
...
def main():
    parser = argparse.ArgumentParser(description="Clam AntiVirus: Local Database Updater ")
    ...
    if not bucket or not prefix:
        parser.error("clamav virus definition bucket and prefix must be provided")
    ...
    log.info(f"Checking for clamav defs updates to {target} s3 bucket: {bucket} prefix: {prefix}")
    while True:
        clamav_defs_updater(bucket, prefix, download, uri, accesskey, secretkey)
        deepsleep(checks)
